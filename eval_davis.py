...
def eval_video_tracking_davis(args, model, scale_factor, frame_list, video_dir, first_seg, seg_ori, color_palette): 
    ...
    # first frame
    frame1, ori_h, ori_w = read_frame(frame_list[0], scale_size=[args.img_size])
    # print(frame1.shape)
    # extract first frame feature
    frame1_feat = extract_feature(args, model, frame1, ori_h, ori_w).T #  dim x h*w
    ...
    for cnt in tqdm(range(1, len(frame_list))): 
        frame_tar = read_frame(frame_list[cnt], scale_size=[args.img_size])[0]

        # we use the first segmentation and the n previous ones
        used_frame_feats = [frame1_feat] + [pair[0] for pair in list(que.queue)]
        used_segs = [first_seg] + [pair[1] for pair in list(que.queue)]

        frame_tar_avg, feat_tar, mask_neighborhood = label_propagation(args, model, frame_tar, used_frame_feats, used_segs, ori_h, ori_w, mask_neighborhood)
...
def label_propagation(args, model, frame_tar, list_frame_feats, list_segs, ori_h, ori_w, mask_neighborhood=None):
    """
    ...
    feat_tar, h, w = extract_feature(args, model, frame_tar, ori_h, ori_w, return_h_w=True)
...
def extract_feature(args, model, frame, ori_h, ori_w, return_h_w=False):
    """Extract one frame feature everytime."""
    
    with torch.no_grad(): 
        dit_feat_raw, dit_ada = model.forward(args,
                                            frame,
                                            timestep=args.t,
                                            block_idx=args.k,
                                            ensemble_size=args.ensemble_size) # c, h, w
        
        B,C,H,W = dit_feat_raw.shape
        # channel discard
        if args.cd:
            dit_feat_raw[:,154,:,:]=0.0
            dit_feat_raw[:,1446,:,:]=0.0
        
        dit_feat = rearrange(dit_feat_raw, "b c h w -> b (h w) c")
        dit_feat = pre_norm(dit_feat)
        dit_feat = rearrange(dit_feat, "b (h w) c -> b c h w", h=H, w=W)
        
        ada_shift = dit_ada[0][0]
        ada_scale = dit_ada[0][1]
        
        ada_shift = ada_shift.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        ada_scale = ada_scale.unsqueeze(0).unsqueeze(2).unsqueeze(3)
        
        
        dit_feat = (1 + ada_scale) * dit_feat + ada_shift
        dit_feat = dit_feat.squeeze(0)
        dim, h, w = dit_feat.shape
        dit_feat = torch.permute(dit_feat, (1, 2, 0)) # h,w,c
        dit_feat = dit_feat.view(h * w, dim) # hw,c
        if return_h_w: 
            return dit_feat, h, w
        return dit_feat
...
def read_frame(frame_dir, scale_size=[960]):
    """
...
def read_seg(seg_dir, scale_factor, scale_size=[960]):
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--dit_model', choices=['sd', 'adm'], default='sd', help="which dift version to use")
    parser.add_argument('--t', default=260, type=int, help='t for diffusion')
    ...
    parser.add_argument('--ensemble_size', default=1, type=int, help='ensemble size for getting an image ft map')
    ...
    parser.add_argument('--output_dir', type=str, help='Path where to save segmentations'
                        , default="./davis_results_flux/")
    parser.add_argument('--data_path', type=str, help="path to davis dataset",
                        default="/mnt/nvme0n1/chaofan/dataset/davis17/davis-2017/DAVIS")
    ...
    for i, video_name in enumerate(video_list): 
        ...
        first_seg, seg_ori = read_seg(seg_path, scale_factor, scale_size=[args.img_size])
